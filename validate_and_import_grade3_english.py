...
def validate_and_import():
    json_file_path = os.path.join('G3', 'grade-3-english.json')
    ...
    subject = data.get('subjectName') # Changed from 'subject' to 'subjectName' to match JSON key I used
        ...
        for strand_data in data.get('strands', []):
            ...
            for sub_data in strand_data.get('subStrands', []):
                ...
                if not sub_strand:
                    ...
                    db.add(sub_strand)
                    print(f"    Created Sub-strand: {sub_id} - {sub_title}")
                else:
                    ...
                    print(f"    Updated Sub-strand: {sub_id} - {sub_title}")
