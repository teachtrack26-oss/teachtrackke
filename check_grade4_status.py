...
def check_grade_4_status():
    session = SessionLocal()
    try:
        templates = session.query(CurriculumTemplate).filter(CurriculumTemplate.grade == "Grade 4").all()
        
        print(f"Found {len(templates)} Grade 4 subjects in database:")
        print("-" * 60)
