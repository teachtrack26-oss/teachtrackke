...
def check_grade_2_status():
    session = SessionLocal()
    try:
        templates = session.query(CurriculumTemplate).filter(CurriculumTemplate.grade == "Grade 2").all()
        
        print(f"Found {len(templates)} Grade 2 subjects in database:")
        print("-" * 60)
