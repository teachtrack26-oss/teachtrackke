...
def validate_and_import():
    json_file_path = os.path.join('G3', 'grade-3-creative-activities.json')
    ...
    print(f"Validating content for Grade {grade} - {subject}")
    ...
    try:
        ...
        # Note: The subject name in DB is "Creative Arts Activities"
        template = db.query(CurriculumTemplate).filter(
            CurriculumTemplate.grade == str(grade),
            CurriculumTemplate.subject == subject
        ).first()
        
        if not template:
            print(f"Error: Curriculum template for Grade {grade} {subject} not found in database.")
            # List available templates for this grade
            templates = db.query(CurriculumTemplate).filter(CurriculumTemplate.grade == str(grade)).all()
            print(f"Available templates for Grade {grade}:")
            for t in templates:
