...
def check_grade_1_status():
    session = SessionLocal()
    try:
        templates = session.query(CurriculumTemplate).filter(CurriculumTemplate.grade == "Grade 1").all()
        
        print(f"Found {len(templates)} Grade 1 subjects in database:")
        print("-" * 60)
