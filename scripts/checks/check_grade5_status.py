...
def check_grade_5_status():
    session = SessionLocal()
    try:
        templates = session.query(CurriculumTemplate).filter(CurriculumTemplate.grade == "Grade 5").all()
        
        print(f"Found {len(templates)} Grade 5 subjects in database:")
        print("-" * 60)
