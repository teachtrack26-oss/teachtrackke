...
def validate_and_import():
    json_file_path = os.path.join('G1', 'grade-1-cre.json')
    ...
    try:
        ...
        # We search by name and grade.
        template = db.query(CurriculumTemplate).filter(
            CurriculumTemplate.grade == f"Grade {grade}",
        ...
        if not template:
            ...
            templates = db.query(CurriculumTemplate).filter(CurriculumTemplate.grade == f"Grade {grade}").all()
