...
export default function ViewSchemePage() {
  ...
  useEffect(() => {
    ...
    if (schemeId && isAuthenticated) {
      fetchScheme();
    }
  }, [schemeId, authLoading, isAuthenticated]);
  ...
  const fetchScheme = async () => {
    try {
      ...
      const status = (s.status === "draft" ? "active" : s.status) ?? "active";
  ...
  const handleDownloadPdf = async () => {
    ...
    const isPremium =
      currentUser?.subscription_type === "INDIVIDUAL_PREMIUM" ||
      currentUser?.subscription_type === "SCHOOL_SPONSORED" ||
  ...
  return (
    <>
      ...
      `}</style>
      <div
        className={`min-h-screen bg-white relative overflow-hidden pt-24 ${
          ...
          <div className="mb-4 flex justify-between items-center no-print">
            ...
            <div className="flex gap-2">
              ...
              >
                ...
                {!isPremium ? "Preview Only" : "Download PDF"}
              </button>
              <button
                onClick={() =>
