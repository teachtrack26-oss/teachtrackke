...
import { FaTrash, FaUserShield, FaUser, FaChartBar, FaSync, FaChevronDown, FaChevronUp } from 'react-icons/fa';
...
export default function AdminUsersPage() {
  ...
  const deleteUser = async (userId: number, userEmail: string) => {
    if (!confirm(`Are you sure you want to delete user ${userEmail}? This will delete all their subjects and progress.`)) {
      return;
    }
  ...
  const resetProgress = async (userId: number, userEmail: string, subjectId?: number) => {
                      ...
                      <tr>
                        <td colSpan={5} className="px-6 py-4 bg-gray-50">
                          <div className="space-y-2">
                            <h4 className="font-semibold text-gray-700 mb-3">Subjects & Progress:</h4>
                            {user.subjects.map((subject) => (
                              <div key={subject.id} className="bg-white p-4 rounded border border-gray-200">
                                <div className="flex items-center justify-between mb-2">
                                  ...
                                  <button
                                    onClick={() => resetProgress(user.id, user.email, subject.id)}
